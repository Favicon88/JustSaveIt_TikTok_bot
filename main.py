...
env = {
    **dotenv_values("/home/JustSaveIt_TikTok_bot/.env.prod"),
...
def tiktok_url_validation(url):
    tiktok_regex = (
        r"(https?://)?(www\.)?"
        "(vt\.)?(tiktok|musical|tiktokcdn)\.(com|ly)/"
        "([^&=%\?]{11})"
    )

    tiktok_regex_match = re.match(tiktok_regex, url)
    if tiktok_regex_match:
        return tiktok_regex_match
...
def download_command(message):
    ...
    if url_info.scheme:
        if url_info.netloc in [
            ...
            if not tiktok_url_validation(url):
